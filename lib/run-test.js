  ...
  return Promise.resolve().then(() => {
    ...
    .then(d => Promise.all([
      new Promise((resolve, reject) => 
        fs.createReadStream(tmpdir+'/test.log').pipe(fs.createWriteStream(logdir+'/test.log'))
          .on('log', console.log)
          .on('error', reject)
          .on('finish', resolve)),
      new Promise((resolve, reject) =>
        fs.createReadStream(tmpdir+'/selenium.log').pipe(fs.createWriteStream(logdir+'/selenium.log'))
          .on('log', console.log)
          .on('error', reject)
          .on('finish', resolve)),
      ])
