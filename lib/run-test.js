...
module.exports = (options) => {
  ...
  return Promise.resolve().then(() => {
    ...
    .then(() => fs.readFileAsync(path.join(__dirname,'../cert/'+options.project+'/'+options.NODE_ENV+'-api.p12'))
      ...
      .then(() => {
        var j = request.jar();
        var passphrase = env.get(options.req.query.username+'/'+options.req.query.reponame+':apiCertPassword:'+options.NODE_ENV);
        return request.getAsync({
          ...
          .then(() => request.getAsync({ uri: 'https://' + options.server + '/auth/token', jar: j, json: true, pfx: p12, passphrase: passphrase
          })
            .spread((res,body) => {
              token = body;
            }))
          .then(() => request.getAsync({ uri: 'https://' + options.server + '/api/utils.integrity.all',jar: j,json:true,body:{token:token}, pfx: p12, passphrase: passphrase })
            .spread((res,body) => {
              if (body && body['utils.integrity.all'] !== 'OK')
                throw 'INTEGRITY_CHECK_FAILED ' + JSON.stringify(body['utils.integrity.all']);
            }));
        });
      })))
    .then(() => mkdirp(tmpdir))
    ...
    .then(() => new Promise((resolve, reject) => {
      var cmd = 'docker load < '+tmpdir+'/selenium.tar.gz' +
        ...
        ' -e VIRTUAL_HOST='+options.req.query.username+'-'+options.req.query.reponame+'-'+
        options.req.query.branchname+'.'+env.get('hostname')+
        ' '+options.dockerTag+
