...
module.exports = (options) => {
  ...
  return Promise.resolve().then(() => {
    ...
    .then(() => fs.readFileAsync(path.join(__dirname,'../cert/'+options.project+'/'+options.NODE_ENV+'.p12'))
      ...
      .then(() => {
        var j = request.jar();
        var passphrase = env.get(options.req.query.username+'/'+options.req.query.reponame+':certPassword:'+options.NODE_ENV);
        return request.getAsync({
          ...
          .then(res => request.getAsync({ uri: 'https://' + options.server + '/auth/token', jar: j, json: true, pfx: p12, passphrase: passphrase
          })
            .then(res => {
              token = res.body;
            }))
          .then(() => request.getAsync({ uri: 'https://' + options.server + '/api/utils.integrity.all',jar: j,json:true,body:{token:token}, pfx: p12, passphrase: passphrase })
            .then(res => {
              if (res.body && res.body['utils.integrity.all'] !== 'OK')
